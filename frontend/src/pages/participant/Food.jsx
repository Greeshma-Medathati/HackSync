...
export default function Food() {
  ...
  const handleMealSelection = async (meal) => {
    try {
      const response = await participantService.getQRForFood(meal);
      if (response.success) {
        setQrCodeData(response.message); // Store Base64 QR code
        setSelectedMeal(meal); // Set the selected meal to open modal
      } else {
        console.error("Failed to get QR code:", response);
      }
    } catch (error) {
          ...
          {meals.map((meal) => (
            ...
            >
              ...
              <div 
                ...
                onClick={() => !isMealTaken(meal) && handleMealSelection(meal)}
              >
          ...
          <div className="bg-[#132D46] p-4 sm:p-8 rounded-lg max-w-sm w-full border-2 border-[#01C38D]">
            ...
            <div className="bg-white p-4 rounded-lg mb-4 flex justify-center">
              {qrCodeData ? (
                ...
                <p className="text-[#191E29] text-center">Loading QR code...</p>
              )}
            </div>
            <button
